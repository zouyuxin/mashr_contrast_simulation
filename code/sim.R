sim = function(nsamp=100, err_sd=1){
  ...
  b1 = rnorm(nsamp)
  B.11 = matrix(cbind(0,0,b1,b1,0,0), nrow = nsamp, ncol = ncond)

  b2 = rnorm(nsamp)
  B.111 = matrix(cbind(0, 0, 0, 0, b2, b2), nrow = nsamp, ncol = ncond)
  B = rbind(B.0, B.1, B.2, B.11, B.111)
...
sim2 = function(nsamp=100, err_sd=1){
  ncond=6
  B.0 = matrix(0, nrow = nsamp, ncol = ncond)
